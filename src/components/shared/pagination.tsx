...
const Pagination = ({ allItems, currentItems = 0, limit, offset, newPage, prevPage, setOffset, params = true }: IPagination) => {
  ...
  return (
    <nav className="flex w-full gap-2 items-center flex-column flex-wrap md:flex-row justify-between" aria-label="Table navigation">
      <span
        className="text-sm font-normal text-primary text-light-text-secondary dark:text-dark-text-secondary mb-4 md:mb-0 block w-full md:inline md:w-auto"
      >
      ...
      <ul className="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8  rounded-md overflow-hidden border ">
        <li>
          <button type='button' onClick={prevPage} className=" bg-primary text-primary-foreground hover:bg-primary/90 flex items-center justify-center px-3 h-8 leading-tight text-light-text-secondary dark:text-dark-text-secondary hover:bg-light-border dark:hover:bg-dark-border hover:text-light-text-primary dark:hover:text-dark-text-primary border-r">Anterior</button>
        </li>
          ...
          <li key={item}>
            <button
              ...
              className={`${offset / limit === item
                ...
                : 'bg-secondary text-primary text-light-text-secondary dark:text-dark-text-secondary'}
        ...
        <li>
          <button type='button' onClick={newPage} className="flex items-center justify-center px-3 h-8 leading-tight bg-primary text-primary-foreground hover:bg-primary/90 dark:text-dark-text hover:bg-light-border dark:hover:bg-dark-border hover:text-light-text-primary dark:hover:text-dark-text-primary">Siguiente</button>
        </li>
      </ul>
    </nav >
  )
